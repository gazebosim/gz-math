...
#   def test_noise(self):
#     gmp = GaussMarkovProcess(20.2, 0.1, 0.0, 0.5)
#     random.seed(1001)
...
#     for i in range(200):
#       value = gmp.Update(0.1)
#       #Hand-tuned vales thaat are repeatable given the seed set above.
#       self.assertGreater(value, -11)
#       self.assertLess(value, 22)

#     if __APPLE_:
#         self.assertAlmostEqual(-4.960893, gmp.Value(), delta=1e-4)
#     elif __MSC_VER :
#         self.assertAlmostEqual(-4.960893, gmp.Value(), delta=1e-4)
#     else :
#         self.assertAlmostEqual(-4.118732, gmp.Value(), delta=1e-4)
