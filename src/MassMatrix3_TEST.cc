...
/////////////////////////////////////////////////
TEST(MassMatrix3dTest, SetFromCapsuleZ_Defaults)
{
  ...
  // input is all zeros, so SetFromCapsuleZ should fail
  EXPECT_FALSE(m.SetFromCapsuleZ(0, 0, 0, q0));
  EXPECT_FALSE(m.SetFromCapsuleZ(0, 0, q0));

  // even if some parameters are valid, none should be set if they
  // are not all valid
  EXPECT_FALSE(m.SetFromCapsuleZ(1, 0, 0, q0));
  EXPECT_FALSE(m.SetFromCapsuleZ(1, 1, 0, q0));
  EXPECT_FALSE(m.SetFromCapsuleZ(1, 0, 1, q0));
  EXPECT_DOUBLE_EQ(m.Mass(), 0.0);
}

/////////////////////////////////////////////////
TEST(MassMatrix3dTest, SetFromCapsuleZ_AlmostSphere)
{
  const math::Quaterniond q0 = math::Quaterniond::Identity;

  // Capsule with short length, very close to a sphere, mass 1.0
  const double mass = 1.0;
  const double length = 1.e-6;
  const double radius = 0.5;
  math::MassMatrix3d m;
  EXPECT_TRUE(m.SetFromCapsuleZ(mass, length, radius, q0));

  const double ixx = 0.4 * mass * std::pow(radius, 2);
  const double iyy = ixx;
  const double izz = ixx;
  const math::Vector3d ixxyyzz(ixx, iyy, izz);
  EXPECT_EQ(m.DiagonalMoments(), ixxyyzz);
  EXPECT_EQ(m.OffDiagonalMoments(), math::Vector3d::Zero);

  const double volume = IGN_PI * radius * radius * (length + 4. / 3. * radius);
  const double density = mass / volume;
  math::Material mat(density);
  EXPECT_DOUBLE_EQ(density, mat.Density());
  math::MassMatrix3d m1;
  EXPECT_FALSE(m1.SetFromCapsuleZ(math::Material(0), length, radius));
  EXPECT_TRUE(m1.SetFromCapsuleZ(mat, length, radius));
  EXPECT_EQ(m, m1);

  // double the length and radius
  EXPECT_TRUE(m.SetFromCapsuleZ(mass, 2*length, 2*radius, q0));
  EXPECT_EQ(m.DiagonalMoments(), 4*ixxyyzz);
}

/////////////////////////////////////////////////
TEST(MassMatrix3dTest, SetFromCapsuleZ_2x1)
{
  const math::Quaterniond q0 = math::Quaterniond::Identity;

  // Capsule with length equal diameter, mass 1.0
  const double mass = 1.0;
  ...
  const double cylinderVolume = IGN_PI * radius2 * length;
  const double sphereVolume = IGN_PI * radius2 * 4. / 3. * radius;
  const double volume = cylinderVolume + sphereVolume;

  const double cylinderMass = cylinderVolume / volume;
  const double sphereMass = sphereVolume / volume;
  ...
  double ixx = cylinderMass / 12.0 * (3*radius2 + length2) +
               sphereMass * (0.4*radius2 + 0.375*radius*length + 0.25*length2);
  double iyy = ixx;
  double izz = radius2 * (0.5 * cylinderMass + 0.4 * sphereMass);
  const math::Vector3d ixxyyzz(ixx, iyy, izz);
  EXPECT_EQ(m.DiagonalMoments(), ixxyyzz);
  EXPECT_EQ(m.OffDiagonalMoments(), math::Vector3d::Zero);
