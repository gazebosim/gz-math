...
class TestMaterial(unittest.TestCase):
    ...
    def test_init(self):
        mats = gz.math9.Material.predefined()
        ...
        self.assertEqual(gz.math9.MaterialType.UNKNOWN_MATERIAL,
                         gz.math9.MaterialType(len(mats)))

        # Iterate over each element in the enum. Check the that enum value
        # matches the type value in the mats map.
        for i in range(gz.math9.MaterialType.UNKNOWN_MATERIAL):
            # Get the type of the material for MaterialType i.
            self.assertEqual(gz.math9.MaterialType(i),
                             mats[gz.math9.MaterialType(i)].type())

            # The name should not be empty
            self.assertTrue(mats[gz.math9.MaterialType(i)].name())

            # The density should be less than the max double value and greater than
            # zero.
            self.assertLess(mats[gz.math9.MaterialType(i)].density(), sys.float_info.max)
            self.assertGreater(mats[gz.math9.MaterialType(i)].density(), 0.0)

        malicious = gz.math9.Material(gz.math9.MaterialType(42))
        self.assertEqual(-1.0, malicious.density())
        self.assertEqual('', malicious.name())

        byDensity = gz.math9.Material(42.2)
        self.assertEqual(42.2, byDensity.density())
        self.assertEqual(gz.math9.MaterialType.UNKNOWN_MATERIAL, byDensity.type())

    def test_comparison(self):
        aluminum = gz.math9.Material(gz.math9.MaterialType.ALUMINUM)

        modified = gz.math9.Material(aluminum)
        ...
        modified.set_type(gz.math9.MaterialType.PINE)
        self.assertNotEqual(modified, aluminum)

    def test_accessors(self):

        mat = gz.math9.Material("Aluminum")
        mat1 = gz.math9.Material("aluminum")
        mat2 = gz.math9.Material(gz.math9.MaterialType.ALUMINUM)
        mat3 = gz.math9.Material(mat2)
        ...
        # Test constructor
        mat4 = gz.math9.Material(mat3)
        self.assertEqual(mat2, mat4)

        mat5 = gz.math9.Material(mat4)
        self.assertEqual(mat2, mat5)

        mat = gz.math9.Material("Notfoundium")
        self.assertGreater(0.0, mat.density())
        self.assertEqual(gz.math9.MaterialType.UNKNOWN_MATERIAL,
                         mat.type())
        self.assertFalse(mat.name())

        material = gz.math9.Material()
        material.set_to_nearest_density(19300.0)
        self.assertEqual(gz.math9.MaterialType.TUNGSTEN, material.type())
